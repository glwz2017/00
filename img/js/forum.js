...
//    动态添加热门标签
    ...
    function createLabe(that) {
        ...
        $(that).parent().before('<li class="ui-item ui-lists"><input type="hidden" value=' + $.trim(that.value.replace(',', ' ').replace('，', ' ')) + '><span class="ui-text">' +
            $.trim(that.value.replace(',', ' ').replace('，', ' '))
