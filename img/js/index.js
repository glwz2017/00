...
//  导航吸顶
    $(window).scroll(function () {
        ...
        if (navTop >= 130) {
            $('.nav-box').css({position: 'fixed', top: 0, left: 0, zIndex: 9999, boxShadow: '0 0 5px #ddd'});
        } else {
            $('.nav-box').css({position: 'relative', zIndex: 9999, boxShadow: 'none'});
        }
    });
//        表单3选择型号：
...
//表单切换
    $('.form-title>ul>li').click(function () {
        var n = $(this).index();
    ...
    $('.js-layer-title li').click(function () {
        ...
        $('.js-layer-img .img-item').eq(index).show().siblings().hide();
    });
